...
try: # Check if the requrements have been installed
    ...
    input(f"Modulo discord_webhook nao instalado, para instalar execute '{'py -3' if os.name == 'nt' else 'python3.8'} -m pip install discord_webhook'\nPressione Enter Para SAIR") # Tell the user it has not been installed and how to install it
...
try: # Setup try statement to catch the error
    ...
    input(f"Modulo requests nao instalado, para instalar execute '{'py -3' if os.name == 'nt' else 'python3.8'} -m pip install requests'\nPressione Enter Para SAIR")# Tell the user it has not been installed and how to install it
    exit() # Exit the program
...
class NitroGen: # Initialise the class
    ...
    def main(self): # The main function contains the most important code
        ...
        if os.name == "nt": # If the system is windows
            print("")
            ctypes.windll.kernel32.SetConsoleTitleA("Gerador e Checker de Nitro - Made by GuiUnder") # Change the
        else: # Or if it is unix
            print(f'\33]0;Gerador e Checker de Nitro - Made by GuiUnder\a', end='', flush=True) # Update title of command prompt
        ...
        self.slowType("Gerador De Nitro Privado GuiUnder", .02) # Print who developed the code
        time.sleep(1) # Wait a little more
        self.slowType("\nColoque a quantidade de nitros gerados e checados: ", .02, newLine = False) # Print the first question

        num = int(input('')) # Ask the user for the amount of codes

        # Get the webhook url, if the user does not wish to use a webhook the message will be an empty string
        self.slowType("\nVocê usa discord webhook no seu servidor? \nColoque aqui, ou aperte enter para ignorar: ", .02, newLine = False)
        ...
        for i in range(num): # Loop over the amount of codes to check
            try: # Catch any errors that may happen
                ...
                print(f" Erroro | {url} ") # Tell the user an error occurred

            if os.name == "nt": # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleA(f"Gerador e checker de nitro - {len(valid)} Valid | {invalid} Invalid - Made by GuiUnder") # Change the title
                print("")
            else: # If it is a unix system
                print(f'\33]0;Gerador e checker de nitro - {len(valid)} Valid | {invalid} Invalid - Made by GuiUnder\a', end='', flush=True) # Change the title

        print(f"""
Results:
 Valido: {len(valid)}
 Invalido: {invalid}
 Nitro Valido: {', '.join(valid )}""") # Give a report of the results of the check
    ...
    def generator(self, amount): # Function used to generate and store nitro codes in a seperate file
        with open(self.fileName, "w", encoding="utf-8") as file: # Load up the file in write mode
            print("Aguarde, estamos gerando para você!") # Let the user know the code is generating the codes
            ...
            print(f"Gerado {amount} codigos | Tempo: {round(time.time() - start, 5)}s\n") #
                ...
                if response.status_code == 200: # If the responce went through
                    ...
                    if notify is not None: # If a webhook has been added
                        DiscordWebhook( # Send the message to discord letting the user know there has been a valid nitro code
                            url = notify,
                            content = f"Um Nitro Valido foi detectado! @everyone \n{nitro}"
                        ).execute()
    ...
    def quickChecker(self, nitro, notify = None): # Used to check a single code at a time
        ...
        if response.status_code == 200: # If the responce went through
            print(f" Valido | {nitro} ", flush=True, end="" if os.name == 'nt' else "\n") # Notify the user the code was valid
            ...
            print(f" Invalido | {nitro} ", flush=True, end="" if os.name == 'nt' else "\n") # Tell the user it tested a code and it was invalid
